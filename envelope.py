...
IGNORE_COMBINATIONS = {
    'Garage Attached Wall': [
        {'Finish Type': 'fiber cement siding', 'Construction Type': 'WoodStud'}
        ],
        'Garage Wall' : [
            {'Finish Type':'fiber cement siding', 'Construction Type': 'WoodStud'}
        ],
...
def get_boundary_rc_values(all_bd_properties, raise_error=False, **house_args):
    ...
    for bd_name in list(all_bd_properties.keys()):
        ...
        # Midrar Added
        ignore_list = IGNORE_COMBINATIONS.get(bd_name, [])
        should_ignore = any (
            (bd_properties.get('Construction Type') == combo.get('Construction Type')
                and bd_properties.get('Finish Type') == combo.get('Finish Type'))
                for combo in ignore_list)
        
        if should_ignore:
            print("\n\n\n")
            print("="*50)
            print(f"[OCHRE WARNING] Ignoring {bd_name} with "
                    f"{bd_properties.get('Construction Type')}, {bd_properties.get('Finish Type')} "
                    f"(unsupported in OCHRE material database).")
