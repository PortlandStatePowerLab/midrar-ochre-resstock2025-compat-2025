...
## Fixes:
As of today's date (12/13/2025), the fixes in this repo only consider upgrade 00 (up00). Other upgrades have not yet to be processed.
...
### tl;dr:
- Copy the envelope.py, hpxml.py, and schedule.py files in this directory to OCHRE installation directory:
...
### Fix:
Within **schedule.py**, ochres incorporates an ignore dictionary (within a lookup table). Here is the original ignore dictionary:
