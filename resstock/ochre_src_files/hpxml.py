...
def parse_hpxml_boundaries(hpxml, return_boundary_dicts=False, **kwargs):
    ...
    # Midrar Added:
    def _area (boundary_name: str):
        return boundaries.get(boundary_name, {}).get('Area (m^2)', []) or []
    
    attic_floor_areas = _area(boundary_name='Attic Floor')
    adj_ceiling_areas = _area(boundary_name='Adjacent Ceiling')
    roof_areas = _area(boundary_name='Roof')

    # End of Midrar Addition
    ...
    # Midrar Added
    elif len(attic_floor_areas) >= 1:
        top_floor_area = sum(attic_floor_areas)
    elif len(adj_ceiling_areas) >= 1:
        top_floor_area = sum(adj_ceiling_areas)
    elif len(roof_areas) >= 1:
        top_floor_area = sum(roof_areas)
    # End of Midrar Addition
    else:
